...
import {
  ExtensionActionRenderIconBtn,
  ExtensionActionRenderTextBtn,
} from "@/extensions/actions/renders/btn/ExtensionActionRenderBtn";
...
type MentionToolbarBtn = {
  char: string;
  title: string;
  tooltip: string | null;
};

function extractMenuDefinition(suggestion: any) {
...
function extractBtnDefinition(suggestion: any): MentionToolbarBtn | undefined {
  let result: MentionToolbarBtn | undefined = undefined;

  if (suggestion.button) {
    result = {
      char: (suggestion.char || "@") as string,
      title: suggestion.button.title,
...
export default class Mention extends AbstractExtension {
  btnDefinitions: MentionToolbarBtn[] = [];
      ...
      : [
          {
            render: new ExtensionActionRenderMenu({
              tooltip: new I18nText("extensions.Mention.buttons.tooltip"),
              icons: {
                [VuetifyIconsGroups.md]: new VuetifyIcon("add"),
                [VuetifyIconsGroups.fa]: new VuetifyIcon("fas fa-plus"),
                [VuetifyIconsGroups.mdi]: new VuetifyIcon("mdi-plus"),
                [VuetifyIconsGroups.mdiSvg]: new VuetifyIcon(
                  "M19,13H13V19H11V13H5V11H11V5H13V11H19V13Z",
                ),
              },
              actions: this.btnDefinitions.map((btnDef) => {
                return {
